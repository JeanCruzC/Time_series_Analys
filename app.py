...
# Hacemos una copia para el análisis extra
df_main = df.copy()
...
if vista == "Día":
    ...
    st.subheader("📆 Curva horaria concatenada por Semana (hueco entre semanas)")

    # Agregamos y construimos dt_week
    wk = (df.groupby(['anno','semana_iso','monday','intervalo'])
            [['planificados','reales']].sum().reset_index())
    wk['dt_week'] = wk.apply(lambda r: r['monday'] +
                                     datetime.timedelta(
                                       hours=r['intervalo'].hour,
                                       minutes=r['intervalo'].minute), axis=1)

    # Insertamos hueco NaN al final de cada semana
    bloques = []
    for _,grupo in wk.groupby(['anno','semana_iso']):
        g = grupo.sort_values('dt_week')
        bloques.append(g)
        gap = g.tail(1).copy()
        gap['dt_week'] += pd.Timedelta(minutes=1)
        gap[['planificados','reales']] = np.nan
        bloques.append(gap)
    wk_gap = pd.concat(bloques, ignore_index=True)

    fig2 = px.line(
        wk_gap, x='dt_week', y=['planificados','reales'],
        labels={'dt_week':'Semana – Hora','value':'Volumen','variable':'Tipo'},
        color_discrete_map={'planificados':'orange','reales':'blue'},
        title="Semana: Zoom & Scroll"
    )
...
# Aseguramos que dia_semana existe en df_main
df_main['dia_semana'] = pd.Categorical(df_main['dia_semana'], categories=dias_orden, ordered=True)
