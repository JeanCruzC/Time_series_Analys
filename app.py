...
if vista == 'Día':
    fig = px.line(serie_continua.reset_index(), x='_dt', y=['planificados','reales'], title='📅 Contactos Día')
    ...
    fig_anom = px.line(serie_continua.reset_index(), x='_dt', y='planificados', title='🔴 Anomalías Día')
    ...
    fig = px.line(serie_last.reset_index(), x='_dt', y=['planificados','reales'], title=f'📆 Contactos Semana ISO {ultima_sem}')
    st.plotly_chart(fig, use_container_width=True)
    decomp = seasonal_decompose(serie_last['planificados'], model='additive', period=24)
    resid = decomp.resid.dropna(); sigma = resid.std(); anoms = resid[np.abs(resid) > 3*sigma]
    fig_anom = px.line(serie_last.reset_index(), x='_dt', y='planificados', title='🔴 Anomalías Semana')
