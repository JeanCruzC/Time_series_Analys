...
if vista=="Día":
    ...
    fig = px.line(ag, x='dt', y=['planificados','reales'],
                  ...
                  line_shape='linear')
    # formato de eje X con fecha / hora en dos líneas
    ...
    if vista=="Semana":
        ag = df.groupby(['semana_iso','nombre_mes'])[['planificados','reales']].sum().reset_index()
    ...
    fig = px.line(ag, x=xcol, y=['planificados','reales'],
                  labels={'value':'Volumen',xcol:vista,'variable':'Tipo'},
                  color_discrete_map={'planificados':'orange','reales':'blue'},
                  title=title)
    fig.update_traces(line=dict(width=2))
    fig.update_xaxes(tickangle=-45)

# 4. Layout común + botón Autoscale
fig.update_layout(
    hovermode="x unified",
    dragmode="zoom",
...
st.download_button(
    ...
    data=aj.to_csv(index=False), file_name="ajustes_2306.csv", mime="text/csv"
)
